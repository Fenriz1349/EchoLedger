...
final class AddTransactionTests: XCTestCase {
    ...
    // MARK: Helpers
    /// Returns a valid split for a given transaction id and account id.
    private func makeSplit(transactionId: UUID = UUID(), accountId: UUID = UUID(), amount: Decimal = 30) -> TransactionSplit {
        TransactionSplit(transactionId: transactionId, accountId: accountId, amount: amount)
    }
    ...
    func test_execute_emptyLabel_throwsEmptyLabel() async {
        ...
        await XCTAssertThrowsErrorAsync(
            try await useCase.execute(label: "", date: Date(), totalAmount: 30, isExpense: true, type: .restaurant, splits: [split])
        ) { error in
    ...
    func test_execute_zeroTotalAmount_throwsInvalidTotalAmount() async {
        ...
        await XCTAssertThrowsErrorAsync(
            try await useCase.execute(label: "Test", date: Date(), totalAmount: 0, isExpense: true, type: .other, splits: [split])
        ) { error in
    ...
    func test_execute_negativeTotalAmount_throwsInvalidTotalAmount() async {
        ...
        await XCTAssertThrowsErrorAsync(
            try await useCase.execute(label: "Test", date: Date(), totalAmount: -10, isExpense: true, type: .other, splits: [split])
        ) { error in
            XCTAssertEqual(error as? TransactionError, .invalidTotalAmount)
        }
    }
    ...
    func test_execute_emptySplits_throwsMissingSplits() async {
        await XCTAssertThrowsErrorAsync(
            try await useCase.execute(label: "Test", date: Date(), totalAmount: 30, isExpense: true, type: .other, splits: [])
        ) { error in
    ...
    func test_execute_zeroSplitAmount_throwsInvalidSplitAmount() async {
        let split = makeSplit(amount: 0)
        await XCTAssertThrowsErrorAsync(
            try await useCase.execute(label: "Test", date: Date(), totalAmount: 30, isExpense: true, type: .other, splits: [split])
        ) { error in
    ...
    func test_execute_multipleSplits_sumMatchesTotalAmount_succeeds() async throws {
        let id = UUID()
        let split1 = makeSplit(transactionId: id, amount: 15)
        let split2 = makeSplit(transactionId: id, amount: 15)
        try await useCase.execute(
            label: "Restaurant",
            date: Date(),
            totalAmount: 30,
